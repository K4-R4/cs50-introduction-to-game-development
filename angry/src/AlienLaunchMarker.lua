...
function AlienLaunchMarker:init(world)
    ...
    -- whether the alian has already splited
    self.split = false
...
function AlienLaunchMarker:splitAlien()
    if not self.split then
        ...
        local xVelo, yVelo = self.aliens[1].body:getLinearVelocity()

        local topAlien = Alien(self.world, 'round', xPos, yPos, 'Player')
        topAlien.body:setLinearVelocity(xVelo, yVelo - math.random(15, 25))
        table.insert(self.aliens, topAlien)

        local bottomAlien = Alien(self.world, 'round', xPos, yPos, 'Player')
        bottomAlien.body:setLinearVelocity(xVelo, yVelo + math.random(15, 25))
