...
function VictoryState:update(dt)
    ...
    if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
        local generatedBricks = LevelMaker.createMap(self.level + 1)
        local hasLockedBricks = false
        for k, brick in pairs(generatedBricks) do
            if not brick.isBreakable  then
        ...
        gStateMachine:change('serve', {
            ...
            hasLockedBricks = hasLockedBricks,
            paddle = self.paddle,
            health = self.health,
            score = self.score,
