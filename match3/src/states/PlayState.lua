...
function PlayState:update(dt)
    ...
    --reset the board if there is no matches available
    if not self:hasPotentialMatches() then
        self.hasMatch = false
        self.board = Board(VIRTUAL_WIDTH - 272, 16, self.level)
        Timer.tween(0.25, {
            [self] = {resetLabelY = VIRTUAL_HEIGHT / 2 - 8}
        }):finish(function()
            Timer.after(0.75, function()
                Timer.tween(0.25, {
                    [self] = {resetLabelY = VIRTUAL_HEIGHT + 30}
                })
            end)
        end)
    end
    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                -- revert swapping if there is no match occuring
                if not self.board:calculateMatches() then
                    local tempX1 = self.highlightedTile.gridX
                    local tempY1 = self.highlightedTile.gridY

                    -- swap grid positions of tiles
                    self.highlightedTile.gridX = newTile.gridX
                    self.highlightedTile.gridY = newTile.gridY
                    newTile.gridX = tempX1
                    newTile.gridY = tempY1
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        -- add score and additional time to timer for each match
...
function PlayState:hasPotentialMatches()
    --check whether each row has potential matches
    for y = 1, 8 do
        local hasMatch = false
        for x = 1, 7 do
            local tile1 = self.board.tiles[y][x]
            local tile2 = self.board.tiles[y][x + 1]
    ...
    --check whether each column has potential matches
    for x = 1, 8 do
        local hasMatch = false
        for y = 1, 7 do
            local tile1 = self.board.tiles[y][x]
            local tile2 = self.board.tiles[y + 1][x]
            -- print("--------")
            -- print(self.board.tiles[y][x].gridX, self.board.tiles[y][x].gridY)
            -- print(self.board.tiles[y][x + 1].gridX, self.board.tiles[y + 1][x].gridY)
...
function PlayState:render()
    ...
    love.graphics.printf('No matches available!', 0, self.resetLabelY, VIRTUAL_WIDTH, 'center')
end